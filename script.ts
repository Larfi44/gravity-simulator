...
class GravitySimulator {
      ...
      link.addEventListener('click', async (e) => {
        ...
        if (isTauri) {
          try {
            ...
            console.error(
              'Failed to open URL via Tauri opener, falling back to window.open:',
              err,
            );
  ...
  private effectiveGravity(currentHeightMeters: number): number {
    if (!this.state.gravityVarEnabled) return this.state.gravity;
  ...
  private animate(): void {
    ...
    for (let step = 0; step < steps; step++) {
      ...
      const currentHeightMeters = Math.max(
        0,
        fraction * this.state.spawnHeight,
      );
...
let appInstance: GravitySimulator | null = null;
